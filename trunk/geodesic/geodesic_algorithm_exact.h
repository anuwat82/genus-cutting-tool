									   ...
									   std::vector<SurfacePoint>* stop_points)
{
	...
	while(!m_queue.empty())
	{
		...
		interval_pointer min_interval = *m_queue.begin();  //get current min interval
		m_queue.erase(m_queue.begin());
		edge_pointer edge = min_interval->edge();
		list_pointer list = interval_list(edge);

		assert(min_interval->d() < GEODESIC_INF);
		int _id0 = edge->adjacent_vertices()[0]->id();
		int _id1 = edge->adjacent_vertices()[1]->id();
																	...
																	interval_pointer source_interval)	//up to two candidates
{
	...
	if(!invert)					//in this case everything is straighforward, we do not have to invert the intervals
	{
		for(unsigned i=0; i<num_candidates; ++i)
		{
			...
			p->edge() = edge;
			if (p->direction() != direction && (p->direction() == Interval::FROM_FACE_0||p->direction() == Interval::FROM_FACE_1 ))
			{
				//p->test_collision = true;
				//p->edge()->source_collision() = true;
			}
...
inline interval_pointer GeodesicAlgorithmExact::best_min_interval_ofEdge(edge_pointer edge, 
											  double& best_total_distance)
{
	list_pointer list = interval_list(edge);
	interval_pointer p = list->first();
...
inline interval_pointer GeodesicAlgorithmExact::best_max_interval_ofEdge(edge_pointer edge, 
											  double& best_total_distance)
{
	list_pointer list = interval_list(edge);
	interval_pointer p = list->first();
...
inline void GeodesicAlgorithmExact::trace_back_edge(edge_pointer destination_edge,		//trace back piecewise-linear edge path
											std::vector<edge_pointer>& edge_path, int level)
{
	...
	edge_path.clear();
	unsigned source_index = std::numeric_limits<unsigned>::max();
	SurfacePoint surPoint(destination_edge,0.1);
...
inline void GeodesicAlgorithmExact::compute_collision_edges()
{
	//anuwat
		...
		if (dirArray[eid] == geodesic::Interval::FROM_FACE_0 || dirArray[eid] == geodesic::Interval::FROM_FACE_1)
		{
			...
			for(unsigned int adj_eid=0; adj_eid< 3; ++adj_eid)
			{
				...
				if (adjOF_edge != _edge && dirArray[adjOF_edge->id()] != geodesic::Interval::FROM_BOTHFACE)
				{
					...
					trace_back_edge(adjOF_edge,edges_path,2);
			...
			for (int adjF = 0 ; adjF < _edge->adjacent_faces().size(); adjF++)
			{
