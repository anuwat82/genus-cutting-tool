...
public:
	...
	{
		m_type = EDGE;
		m_source_collision_flag = 0;
	};

	~Edge(){};

	double& length(){return m_length;};
	int & source_collision() {return m_source_collision_flag;};
	const int & source_collision() const{return m_source_collision_flag;};
	...
	bool isVertices(vertex_pointer v1,vertex_pointer v2)
	{
		if (belongs(v1) && belongs(v2))
			return true;
		else
			return false;
	}
