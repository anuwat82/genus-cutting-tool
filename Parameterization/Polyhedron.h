...
class Polyhedron{
  ...
  */  
  ...
  double resultStretch; 
  
  int allocateVsize,allocateFsize;
  ...
  void param(FILE* logFile = NULL){
    ...
    if(paramtype==1){
      ParametrizationSingle(iteNum,PCBCGerror,logFile);
...
void memorydelete()
{
	...
	if(PHead!=NULL&&PTail!=NULL)
	{
		for(i=0;i<numberV;i++)
		{	  
			if( PHead[i]!=NULL&&PTail[i]!=NULL && PHead[i]->next!=PTail[i])
			{
	...
	{
		IDtool->CleanNeighborL((IDList **)VHead,(IDList **)VTail,numberV);
		VHead=NULL;
		VTail=NULL;
	}
  ...
  void memoryallocate(int dV,int dF){
	...
	memset(point,0x00,sizeof(Point3d*)*numberV);
	memset(Face,0x00,sizeof(int *)*numberF);
    
    IHead = new IDList* [numberV];
    ITail = new IDList* [numberV];    
    VHead = new VList* [numberV];
    VTail = new VList* [numberV];
    FHead= new IDList* [numberV];
    FTail= new IDList* [numberV];
	PHead = new PolarList* [numberV];
    PTail = new PolarList* [numberV];  

	memset(IHead,0x00,sizeof(IDList*)*numberV);
	memset(ITail,0x00,sizeof(IDList*)*numberV);
	memset(VHead,0x00,sizeof(VList*)*numberV);
	memset(VTail,0x00,sizeof(VList*)*numberV);
	memset(FHead,0x00,sizeof(IDList*)*numberV);
	memset(FTail,0x00,sizeof(IDList*)*numberV);
	memset(PHead,0x00,sizeof(PolarList*)*numberV);
	memset(PTail,0x00,sizeof(PolarList*)*numberV);
	...
	memset(boundary,0x00,sizeof(int)*numberV);
	memset(neighborI,0x00,sizeof(int)*numberV);
	memset(neighborF,0x00,sizeof(int)*numberV);
	memset(sigma,0x00,sizeof(double)*numberV);
	/*
    for(i=0;i<numberV;i++){
      boundary[i]=0;
      neighborI[i]=0;
	...
	*/
	...
	memset(areaMap3D,0x00,sizeof(double)*numberF);
	memset(E,0x00,sizeof(double)*numberF);
	memset(G,0x00,sizeof(double)*numberF);
	/*
    for(i=0;i<numberF;i++){
      areaMap3D[i]=0.0;
      E[i]=0.0;
...
void SetBoundaryLines()
{
	...
	if(neighborI!=NULL && neighborF!=NULL && boundary!=NULL)
	{
		...
		for(i=0;i<numberV;i++)
		{
			...
			else if((neighborI[i] == neighborF[i]) && 
				(neighborF[i] !=0) && 
				(neighborI[i] !=0)
			  )
			{
  ...
  VList::AppendVF(dj,i,VTail[di]);
  VList::AppendVF(dk,i,VTail[di]);
  /* Two */
  neighborF[dj]++;
  
  IDtool->AppendISort(di,IHead[dj],ITail[dj],dj,neighborI);
  IDtool->AppendISort(dk,IHead[dj],ITail[dj],dj,neighborI);
  VList::AppendVF(dk,i,VTail[dj]);
  VList::AppendVF(di,i,VTail[dj]);
  /* Three */
  neighborF[dk]++;
  
  IDtool->AppendISort(di,IHead[dk],ITail[dk],dk,neighborI);
  IDtool->AppendISort(dj,IHead[dk],ITail[dk],dk,neighborI);
  VList::AppendVF(di,i,VTail[dk]);
  VList::AppendVF(dj,i,VTail[dk]);


	IDtool->AppendVFSort(i,FHead[di],FTail[di]);
	IDtool->AppendVFSort(i,FHead[dj],FTail[dj]);
	IDtool->AppendVFSort(i,FHead[dk],FTail[dk]);
}
...
void ParametrizationSingle(int itenum,double error,FILE* logFile = NULL)
{
	...
	resultStretch =  getCurrentE();
	if (logFile)
	{
		fprintf(logFile,"U0  STRETCH: %f\n",resultStretch);
	}
...
void ParametrizationOptimal(int itenum,double error,FILE* logFile = NULL)
{
 ...
 // UOpt: Optimal low stretch parameterization (slow and smooth)
  void ParametrizationSmoothOptimal(int itenum,double error,FILE* logFile = NULL){
    double *UaXY = new double[2*(numberV)+1];
    double *vecb = new double[2*(numberV)+1];
