...
First, we remove faces around $v_s$ as seed triangles. Next, we iteratively analyze edges (adjacent only one triangle) in a non-empty queue under the conditions that the edge and its adjacent face on low priority queue are removed first and then the edge with shortest geodesic distance in the queue will be removed first.
...
\begin{table}[h]
	...
	\begin{tabular}{|C{16pt}|C{24pt}|C{22pt}|C{22pt}|C{22pt}|C{22pt}|C{22pt}|C{22pt}|}
		...
		\hline
	\multirow{2}{*}{{\small case}} & \multirow{2}{*}{{\small genus}} & \multicolumn{2}{ c|}{ {\small time (sec)}} & \multicolumn{2}{ c|}{ {\small square} } & \multicolumn{2}{ c|}{{\small circular}} \\\cline{3-8}
