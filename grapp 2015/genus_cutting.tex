...
At the beginning in the method , if the mesh has boundaries, let $\mathscr{B}$  be the set of original boundary edges that remain unchanged in the whole process and will be included in final cut graph ${\rho}$. It first starts by removing a single seed triangle from the mesh. At this moment, each edge of the seed triangle is adjacent to only one triangle respectively (see figure \subref*{fig:OriginalGenusReduceMethodStepByStep-a}).  After removing the seed triangle from the mesh, there are two processing phases.

In the first phase, it repeatedly detects an edge adjacent exactly to one triangle that is not in $\mathscr{B}$, and removes both the edge and the triangle from the mesh structure. The rest two edges are left (see figure \subref*{fig:OriginalGenusReduceMethodStepByStep-b}). If the rest edges of the removing triangle are not adjacent to any triangle, then the edges will become one of candidates of cut graph (see figure \subref*{fig:OriginalGenusReduceMethodStepByStep-c}). Generally, removing one edge and one triangle triggers more two edges to be adjacent to only one triangle further. Because of the above condition, the removing propagation will keep spreading out from the seed triangle according to geodesic distance in order to get minimum radius result (see figure \subref*{fig:OriginalGenusReduceMethodStepByStep-d}). Since a 2-manifold triangle mesh is being processed, any triangle will be removed eventually. Therefore, this phase ends when there is no triangle left and there remain only edges and their vertices as candidate cut graph edges (see figure \subref*{fig:fig-original_genus_reducing_process-c}). At this point, the cut $\rho$ consists of a set of connecting $2g$ loops.
...
\begin{figure}[h!]
	...
	\hspace{0pt}
	\subfloat[\label{fig:geodesic_fail_propagte}non-propagation type]{\includegraphics[width=0.45\columnwidth]{images/fail_propagate.png}}
	\hspace{0.00\columnwidth}
...
First, we detect an edge $e_i$ whose intervals $I_{e_i}$ satisfy the condition: if there is an interval that $f_p$ is not same as other intervals then we consider $e_i \in \tilde{E}$. From figure \subref*{fig:geodesic_both_face}, we can see clearly that $e_a$ has two intervals $i_{e_{a^\alpha}}$ and $i_{e_{a^\beta}}$, where first one has a pseudo-source from $f_l$ while second one has another pseudo-source from $f_r$. Typically, this kind of edges can be found a few in mesh (see figure \subref*{fig:edge from two faces}). 
...
At this stage, $\tilde{E}$ may typically contains separating or contractible cycles (see figure \subref*{fig:after truncate}).  For separating cycles issue, we define all edges in $\hat{E}$ to be neighbors of each edges in $\tilde{E}$. We consider that $(\tilde{E} \cup \hat{E})$ contains $2g$ loops inside it.  
...
\begin{figure*}[th!]
	\centering		
	\subfloat[\label{fig:case01 original}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test01/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case01 proposed}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test01/propose.png}}
	\hspace{10pt}
	\subfloat[\label{fig:case02 original}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test02/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case02 proposed}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test02/propose.png}}\\
	
	\subfloat[\label{fig:case03 original}]{\includegraphics[width=0.40\columnwidth]{images/experiment/test03/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case03 proposed}]{\includegraphics[width=0.40\columnwidth]{images/experiment/test03/propose.png}}	
	\hspace{10pt}		
	\subfloat[\label{fig:case04 original}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test04/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case04 proposed}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test04/propose.png}}\\	
	
	
	\subfloat[\label{fig:case05 original}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test05/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case05 proposed}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test05/propose.png}}
	\hspace{10pt}			
	\subfloat[\label{fig:case06 original}]{\includegraphics[width=0.40\columnwidth]{images/experiment/test06/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case06 proposed}]{\includegraphics[width=0.40\columnwidth]{images/experiment/test06/propose.png}}\\
	
	\subfloat[\label{fig:case07 original}]{\includegraphics[width=0.5\columnwidth, angle=45]{images/experiment/test07/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case07 proposed}]{\includegraphics[width=0.5\columnwidth, angle=45]{images/experiment/test07/propose.png}}
	\hspace{10pt}			
	\subfloat[\label{fig:case08 original}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test08/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case08 proposed}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test08/propose.png}}\\
	
	\subfloat[\label{fig:case09 original}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test09/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case09 proposed}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test09/propose.png}}
	\hspace{10pt}			
	\subfloat[\label{fig:case10 original}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test10/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case10 proposed}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test10/propose.png}}\\
	
	\subfloat[\label{fig:case11 original}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test11/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case11 proposed}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test11/propose.png}}
	\hspace{10pt}			
	\subfloat[\label{fig:case12 original}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test12/original.png}}\hspace{5pt}
	\subfloat[\label{fig:case12 proposed}]{\includegraphics[width=0.45\columnwidth]{images/experiment/test12/propose.png}}\\	
	\caption[]{.......}
