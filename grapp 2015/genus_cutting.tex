...
\author{\authorname{Anuwat Dechvijankit\sup{1}, Hiroshi Nagahashi\sup{2} and Kota Aoki\sup{2}}
\affiliation{\sup{1}Department of Computational Intelligence and Systems Science, Tokyo Institute of Technology, Yokohama, Japan}
\affiliation{\sup{2}Imaging Science and Engineering Laboratory, Tokyo Institute of Technology, Yokohama, Japan}
\email{dechvijankit.a.aa@m.titech.ac.jp, longb@isl.titech.ac.jp, aoki.k.af@m.titech.ac.jp}
}
...
\begin{figure*}[t]
	\centering		
	\subfigure[\label{fig:OriginalGenusReduceMethodStepByStep01}]{\includegraphics[width=0.175\linewidth]{images/gim_progress_01.png}}\hspace{10pt}
	\subfigure[\label{fig:OriginalGenusReduceMethodStepByStep02}]{\includegraphics[width=0.175\linewidth]{images/gim_progress_02.png}}\hspace{10pt}
	\subfigure[\label{fig:OriginalGenusReduceMethodStepByStep03}]{\includegraphics[width=0.175\linewidth]{images/gim_progress_03.png}}\hspace{10pt}
	\subfigure[\label{fig:OriginalGenusReduceMethodStepByStep04}]{\includegraphics[width=0.175\linewidth]{images/gim_progress_04.png}}\hspace{10pt}		
	\subfigure[\label{fig:OriginalGenusReduceMethodStepByStep05}]{\includegraphics[width=0.175\linewidth]{images/gim_progress_05.png}}
...
At the beginning, when the mesh $\mathscr{M}$ has boundaries, let $\mathscr{B}$  be the set of original boundary edges that remain unchanged in the whole process and will be included in final cut graph ${\rho}$. It first starts by removing a single seed triangle from the mesh. At this moment, each edge of the seed triangle is adjacent to only one triangle respectively (see figure~\ref{fig:OriginalGenusReduceMethodStepByStep02}).  After removing the seed triangle from the mesh, there are two processing phases.

In the first phase, it repeatedly detects an edge adjacent exactly to one triangle that is not in $\mathscr{B}$, and removes both the edge and the triangle from the mesh structure. The rest two edges are left (see figure~\ref{fig:OriginalGenusReduceMethodStepByStep03}). If the rest edges of the removing triangle are not adjacent to any triangle, then the edges will become one of candidates of cut graph (see figure~\ref{fig:OriginalGenusReduceMethodStepByStep04}). Generally, removing one edge and one triangle triggers additional two edges to be adjacent to only one triangle further. Because of the above condition, the removing propagation will keep spreading out from the seed triangle according to geodesic distance in order to get minimum radius result. Since a 2-manifold triangle mesh is being processed, every triangle will be removed eventually. Therefore, this phase ends when there is no triangle left and there remain only edges and their vertices as candidate cut graph edges. At this point, the cut $\rho$ consists of a set of connecting $2g$ loops.
...
\begin{figure}[h!]
	\centering		
	\subfigure[\label{fig:fig-original_remove_dangling_edges_step_by_step-a}]{\includegraphics[width=0.45\columnwidth]{images/fig-original_remove_dangling_edges_step_by_step-a.png}} \hspace{10pt}
	\subfigure[\label{fig:fig-original_remove_dangling_edges_step_by_step-b}]{\includegraphics[width=0.45\columnwidth]{images/fig-original_remove_dangling_edges_step_by_step-b.png}}\\		
	\subfigure[\label{fig:fig-original_remove_dangling_edges_step_by_step-c}]{\includegraphics[width=0.45\columnwidth]{images/fig-original_remove_dangling_edges_step_by_step-c.png}}	\hspace{10pt}	
	\subfigure[\label{fig:fig-original_remove_dangling_edges_step_by_step-d}]{\includegraphics[width=0.45\columnwidth]{images/fig-original_remove_dangling_edges_step_by_step-d.png}}	\\
	\subfigure[\label{fig:fig-original_remove_dangling_edges_step_by_step-e}]{\includegraphics[width=0.45\columnwidth]{images/fig-original_remove_dangling_edges_step_by_step-e.png}}	\hspace{10pt}
	\subfigure[\label{fig:fig-original_remove_dangling_edges_step_by_step-f}]{\includegraphics[width=0.45\columnwidth]{images/fig-original_remove_dangling_edges_step_by_step-f.png}}	 
...
First, we detect an edge $e_i$ whose intervals $I_{e_i}$ satisfy the condition: if there is an interval that $f_p$ is not same as other intervals then we consider $e_i \in \tilde{E}$. From figure~\ref{fig:geodesic_both_face}, we can see clearly that $e_a$ has two intervals $i_{e_{a,1}}$ and $i_{e_{a,2}}$, where first one has a pseudo-source from $f_l$ while second one has another pseudo-source from $f_r$. Typically, this kind of edges can be found a few in mesh (green edges in figure~\ref{fig:edge nearby crossing}). 

\begin{figure}[h!]
	\centering		
	\subfigure[\label{fig:geodesic_both_face}from both sides type]{\includegraphics[width=0.48\columnwidth]{images/two_pseudosource.png}}
	\hspace{0pt}
	\subfigure[\label{fig:geodesic_fail_propagte}non-propagation type]{\includegraphics[width=0.48\columnwidth]{images/fail_propagate.png}}
...
\begin{figure*}[bht!]
	\centering		
	%\subfloat[\label{fig:edge from two faces}]{\includegraphics[width=0.75\columnwidth]{images/edge_two_faces.png}}
	%\hspace{20pt}
	\subfigure[\label{fig:edge nearby crossing}]{\includegraphics[width=0.65\columnwidth]{images/edge_nearby_crossing.png}}
	\hspace{6pt}
	\subfigure[\label{fig:after truncate}]{\includegraphics[width=0.67\columnwidth]{images/after_truncation.png}}
	\hspace{6pt}
	\subfigure[\label{fig:neighbor edges}]{\includegraphics[width=0.67\columnwidth]{images/neighbor_edges.png}}	
	
	\caption[]{Processes to define $\tilde{E}$ and $\hat{E}$ on a torus model. \subref{fig:edge nearby crossing} shows green colored edges that their intervals' pseudo-sources are from both sides, and blue colored edges that their intervals have not propagated. \subref{fig:after truncate} shows the graph after removing dangling edges. \subref{fig:neighbor edges} shows $\tilde{E}$ in red color and $\hat{E}$ in  yellow color.}
...
At this stage, $\tilde{E}$ may typically contains separating or contractible cycles (see figure~\ref{fig:after truncate}).  For separating cycles issue, we define all edges in $\hat{E}$ to be neighbors of each edge in $\tilde{E}$. We consider that a set of edges $\rho \equiv (\tilde{E} \cup \hat{E})$ contains $2g$ loops inside it.  
...
\begin{table}[h]
	...
	\begin{tabular}{|C{16pt}|C{18pt}|C{23pt}|C{23pt}|C{23pt}|C{23pt}|C{23pt}|C{23pt}|}
		...
		\hline
	\multirow{2}{*}{{\scriptsize case}} & \multirow{2}{*}{{\scriptsize genus}} & \multicolumn{2}{ c|}{ {\scriptsize time (sec)}} & \multicolumn{2}{ c|}{ {\scriptsize square} } & \multicolumn{2}{ c|}{{\scriptsize circular}} \\\cline{3-8}
		                                   &       &   {\scriptsize original} &   {\scriptsize our}&   {\scriptsize original} &   {\scriptsize our}      &   {\scriptsize original} &   {\scriptsize our}\\
		\hline

		01    & 1  & 0.006 & 0.009 & 1.351   &  \cellcolor{cyan!30}1.335    &  1.493  &  \cellcolor{green!25}1.446 \\
		02    & 1  & 0.014 & 0.030 & 1.492   &  \cellcolor{cyan!30}1.468    &  1.687  &  \cellcolor{green!25}1.663 \\
		03    & 1  & 0.014 & 0.029 & \cellcolor{cyan!30}1.562   &  1.636    &  \cellcolor{green!25}1.800  &  2.102 \\
		04    & 1  & 0.040 & 0.106 & 1.551   &  \cellcolor{cyan!30}1.417    &  2.011  &  \cellcolor{green!25}1.635 \\
		05    & 1  & 0.040 & 0.106 & 1.561   &  \cellcolor{cyan!30}1.362    &  1.924  &  \cellcolor{green!25}1.538 \\
		06    & 2  & 0.028 & 0.051 & 1.667   &  \cellcolor{cyan!30}1.466    &  2.246  &  \cellcolor{green!25}1.701 \\
		07    & 2  & 0.028 & 0.052 & \cellcolor{cyan!30}1.390   &  1.442    &  1.603  &  \cellcolor{green!25} 1.554 \\
		08    & 3  & 0.489 & 1.217 & 1.457   &  \cellcolor{cyan!30}1.452    &  1.688  &  \cellcolor{green!25}1.679 \\
		09    & 3  & 0.527 & 1.228 & 2.041   &  \cellcolor{cyan!30}2.025    &  \cellcolor{green!25}9.279  &  9.827 \\
		10   & 3  & 0.263 & 0.574 & 471.2   &  \cellcolor{cyan!30}1.509    &  4.068  &  \cellcolor{green!25}1.978 \\
		11   & 3  & 0.497 & 1.163 & 939.2   &  \cellcolor{cyan!30}1.625    &  6.348  &  \cellcolor{green!25}4.115 \\
		12   & 3  & 0.110 & 0.142 & 221.3K   &  \cellcolor{cyan!30}79.9K    &  383.2K  &  \cellcolor{green!25}203.4K \\
...
%This study is supported by ****** (Grant Number ******).
...
\begin{figure*}[th!]
	...
	\subfigure[\label{fig:case01}case 01 (vertices 200, faces 400)]{ \centering	
	...
	\hspace{20pt}
	\subfigure[\label{fig:case02}case 02 (vertices 500, faces 1000)]{ \centering
	...
	\subfigure[\label{fig:case03}case 03 (vertices 500, faces 1000)]{ \centering
	...
	\hspace{20pt}
	\subfigure[\label{fig:case04}case 04 (vertices 1152, faces 2304)]{ \centering
	...
	\hspace{20pt}
	\subfigure[\label{fig:case06}case 06 (vertices 766, faces 1536)]{ \centering
	...
	\subfigure[\label{fig:case07}case 07 (vertices 766, faces 1536)]{ \centering
	...
	\hspace{20pt}
	\subfigure[\label{fig:case08}case 08 (vertices 6568, faces 13144)]{
	...
	\subfigure[\label{fig:case09}case 09 (vertices 6568, faces 13144)]{ \centering
	...
	\hspace{20pt}
	\subfigure[\label{fig:case10}case 10 (vertices 4192, faces 8392)]{ \centering
	...
	\subfigure[\label{fig:case11}case 11 (vertices 6652, faces 13312)]{ \centering
	...
	\hspace{20pt}
	\subfigure[\label{fig:case12}case 12 (vertices 2496, faces 5000)]{ \centering
	...
	\caption[]{Results of homotopy cutting from original \cite{Gu:2002:GI:566654.566589} and our approach methods. The meshes having yellow line are results obtained by original approach. The meshes having red line are our results.}
