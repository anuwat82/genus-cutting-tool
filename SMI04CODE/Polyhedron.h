...
class Polyhedron{
  ...
  */  
  ...
  void readmesh(double *vertice , int number_vertices, int* triangleIndices , int numberTriangle)
  {
	  memoryallocate(number_vertices,numberTriangle);
     for(int i=0;i<numberV;i++)
	 {      
		 setPoint(i,vertice[i*3 + 0],vertice[i*3 + 1],vertice[i*3 + 2]);
    }
    int val=3;
    for(int i=0;i<numberF;i++){      
      setFace(i,triangleIndices[i*3 + 0],triangleIndices[i*3 + 1],triangleIndices[i*3 + 2]);
  ...
  void readmesh(char *filename){
    ...
    double dy=0.0;
    double dz=0.0;
    size_t ssize = fscanf_s(in,"%d",&dV);
    ssize = fscanf_s(in,"%d",&dF);
    
    memoryallocate(dV,dF);
    for(i=0;i<numberV;i++){
      ssize =fscanf_s(in,"%lf %lf %lf",&dx,&dy,&dz);
      setPoint(i,dx,dy,dz);
    }
    int val=3;
    for(i=0;i<numberF;i++){
      ssize = fscanf_s(in,"%d %d %d %d",&val,&di,&dj,&dk);
      setFace(i,di,dj,dk);
      IDtool->AppendVFSort(i,FHead[Face[i][0]],FTail[Face[i][0]]);
      IDtool->AppendVFSort(i,FHead[Face[i][1]],FTail[Face[i][1]]);
      IDtool->AppendVFSort(i,FHead[Face[i][2]],FTail[Face[i][2]]);
  ...
  void writemesh_ply(char *filename){
	FILE *out=NULL;
	fopen_s(&out,filename,"wt"); 
	fprintf(out, "ply\n");
	fprintf(out, "format ascii 1.0\n");
	fprintf(out, "comment ply output\n");
...
//#pragma omp parallel for
	for(int i=0;i<numberV;i++)
	{
		if (boundary[i] == 0 )
		{
			...
			{
				IDList *search = next(next(VHead[i]));
				IDList *now = search;				
  ...
  void memorydelete(){
	...
	for(i=0;i<numberV;i++){
	  ...
	  if(boundary[i]==0&&PHead[i]!=NULL&&PTail[i]!=NULL&&PHead[i]->next!=PTail[i]){
...
double getCurrentE(){
  ...
  double l2_stretch = sqrt( sum2Darea*dsum / (sum3Darea*sum3Darea) );
  


  if (l2_stretch==l2_stretch)
	return l2_stretch;
  else
	 return -1.0;
